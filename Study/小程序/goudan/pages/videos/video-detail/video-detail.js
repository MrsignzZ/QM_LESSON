...
Page({
      ...
      .then(res => {
        if (res.data.status === 0) {
          res.data.data.articles.map(v => {
            ...
            v.thumbnails[0].url = v.thumbnails[0].url.replace(
              /(\.\w{3,4})$/i,
              '_crop' + 234 + 'x' + 146 + '$1'
            );
          });
  ...
  openDetail(event) {
    ...
    let url = `../video-detail/video-detail?title=${item.title}&time=${
      item.create_time
    }&url=${item.videos[0].video_src}`;
    // wx.navigateTo({  // 会反复打开新窗口，最多十层。
    //   url: url
    // })
    wx.redirectTo({
      // 关闭当前页面，跳转到应用内的某个页面,不触发OnHide()方法,且能直接返回到tab页面
      url: url
    });
  },
  /**
